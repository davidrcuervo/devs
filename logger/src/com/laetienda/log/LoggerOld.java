...
@Deprecated
public class LoggerOld extends Father {
	...
	public LoggerOld(File directory) throws Exception{
		super(directory);
	...
	private int getLevel(String level) throws IOException{
		...
		int c;
		for(c = 0; c < LoggerOld.getLevels().length; c++){
			//System.out.println(level + " compared with " + Logger.getLevels()[c]);
			if(level.equals(LoggerOld.getLevels()[c])){
				result = c;
				break;
			} 
		}
	...
	private void saveToDb(){
		...
		try{
			trans.insert(trans.getEm(), getEntity());
		}catch (Exception ex){
	...
	private String getLogLine(){
		return Integer.toString(LoggerOld.counter) + "\t"
